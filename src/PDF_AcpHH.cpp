...
PDF_AcpHH::PDF_AcpHH(const TString obs_id, const TString unc_id, const hypotheses::dy_fsc dy_fsc_hypo,
                     const parametrisations::acp acp_param, const parametrisations::mix mix_param, const int nObs,
                     const bool deferInitialise)
    : PDF_Charm{nObs}, dy_fsc_hypo{dy_fsc_hypo}, acp_param{acp_param}, mix_param{mix_param} {
  name = "Charm_AcpHH";
  if (!deferInitialise) initialise(obs_id, unc_id, unc_id);
}
