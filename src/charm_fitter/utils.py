...
class Range:
    ...
    def transformed_range(self, transf: Callable[[float], float]) -> Self:
...
class _Scan:
    ...
    def __post_init__(self):
        ...
        if self.parfile:
            if not (repo_path / self.parfile).is_file():
                raise FileNotFoundError(f"Parfile '{repo_path / self.parfile}' not found")
...
class CharmPlottingConfig(PlottingConfig):
    ...
    Attribute:
        ...
        compare_dcs_hypos: Whether to compare different hypotheses for CP violation D0 -> K+ pi- decays in the plots
            (when the hypothesis differs from the default).
    """
            ...
            for dcs_cpv in dcs_cpv_vals:
                for dy_fsc in plot.dy_fsc:
                    ...
                    plotter.add_scan(
                        ...
                        bf=(plots_cat != "breakdown" or cname in cfg.baseline_combiners),
                    )
                    i += 1
        plotter.plot()
