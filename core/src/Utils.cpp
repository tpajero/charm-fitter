 ...
 *               ...}
 */
TMatrixDSym Utils::buildCorMatrix(const int n, std::vector<double> data) {
    int idata = 0;
    TMatrixDSym m{n};
    if (data.size() == static_cast<typename std::vector<double>::size_type>(n * n)) {
        m = TMatrixDSym(n, &data[0]);
    } else if (data.size() == static_cast<typename std::vector<double>::size_type>(n * (n+1) / 2)) {
        for (int i=0; i<n; ++i) {
            for (int j=0; j<n; ++j) {
        ...
        for (int i=0; i<n; ++i) {
            ...
            std::cerr << "FAILURE (Utils::buildCorMatrix(int, std::vector<double>): inconsistent input data";
            exit(1);
    }
    return m;
}
