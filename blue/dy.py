...
measures = {
    ...
    "cdf-2014": {
        "all": Measurement("CDF 2014", "1410.5435", 12.0, 12.0),
        "kk": Measurement("CDF 2014", "1410.5435", 19.0, 15.0, 4.0),
        "pipi": Measurement("CDF 2014", "1410.5435", 1.0, 18.0, 3.0),
    },
    "belle-2016": {"all": Measurement("Belle 2016", "1509.08266", 3.0, 20.0, 7.0)},
    "lhcb-2015": {
    ...
    "lhcb-2024": {
        "all": Measurement(
            r"LHCb 2024 $D^0\to\pi^{+}\pi^{-}\pi^{0}$ (6 fb$^{-1}$)",
            "2405.06556",
            -1.3,
            6.3,
            2.4,
    ...
    "lhcb-avg-2020": {"all": Measurement("LHCb average", None, -1.0, 1.1, 0.3)},
    "lhcb-avg-2021": {
        "all": Measurement(
            r"LHCb 2024 $D^0\to\pi^{+}\pi^{-}\pi^{0}$ (6 fb$^{-1}$)",
            "2405.06556",
    ...
    "wa-2020": {"all": Measurement("World average", None, 1.9, 1.6, 0.5)},
    "wa-2021": {"all": Measurement("World average", None, -0.9, 1.1, 0.3)},
    "wa-2024": {
...
def get_xy_ranges(comb, final_state, dy_notation):
    ...
    if "lhcb" in comb:
        x_min_lhcb = {
            "all": -10.0 if dy_notation else -23,
            "kk": -10.0 if dy_notation else -25,
            "pipi": -16.0 if dy_notation else -27,
        }
        x_max_lhcb = {
        ...
        x_min_wa = {
            "all": -40.0 if dy_notation else -27,
            "kk": -10.0 if dy_notation else -40,
...
def make_plot(comb, final_state, dy_notation, out_dir):
    ...
    decay_label = (
        "" if final_state == "all" else r"(D^0 \to K^+ K^-)" if final_state == "kk" else r"(D^0 \to \pi^+ \pi^-)"
    )
