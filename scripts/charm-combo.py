...
if __name__ == "__main__":
    ...
    assert isinstance(cfg, CharmPlottingConfig), "Configuration file must define an instance of CharmPlottingConfig"
    COMPARE_DCS_HYPOS = args.dcs_cpv != args.dcs_cpv_default and cfg.compare_dcs_hypos
    ...
    with cwd(repo_path):  # Need to call the executable from the repository root due to GammaCombo core limitations
        if any(x in ["all", "1d"] for x in args.actions):
            ...
            plots_1d(cfg.plots_1d, args, cfg, compare_dcs_hypos=COMPARE_DCS_HYPOS)
        if any(x in ["all", "2d"] for x in args.actions):
            scans_2d(list(cfg.baseline_combiners.values()), args)
            plots_2d(cfg.plots_2d, args, cfg, compare_dcs_hypos=COMPARE_DCS_HYPOS)
        if any(x in ["all", "dy-fsc"] for x in args.actions):
            ...
            plots_1d(cfg.plots_dy_fsc_1d, args, cfg, plots_cat="compare-dy-fsc/1d")
            plots_2d(cfg.plots_dy_fsc_2d, args, cfg, plots_cat="compare-dy-fsc/2d")
