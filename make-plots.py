...
def get_scans_1d(combinations, scan_dy_rs=False):
    ...
    scans.extend(
        [
            Scan1d("Delta_Kpipi0", -2.0, 2.0),
            Scan1d("Delta_K3pi", -0.5, 1.5),
            Scan1d("k_K3pi", 0.0, 1.0),
            Scan1d("k_Kpipi0", 0.0, 1.0),
            Scan1d("r_K3pi", 4.0, 6.0),
            Scan1d("r_Kpipi0", 4.0, 6.0),
            Scan1d("F_pipipi0", 0.9, 1.0),
        ]
    )
    if scan_dy_rs:
        scans.append(Scan1d("DY_RS", 0.0, 0.4))
    return scans
    ...
    for scan in get_scans_1d([int(combo) for combo in combinations]):
        ...
        while i < len(cc):
            if (scan.full_fsc_only and cc[i] not in _full_fsc_combinations) or (
                scan.var == "Acp_KP" and plot_acp_kp and int(cc[i]) >= 1000
            ):
...
def plot_2d(param, combinations, titles, parfile, already_plotted, lhcb, no_dcs_cpv):
    ...
    for scan in get_scans_2d([int(combo) for combo in combinations], plot_acp_kp=plot_acp_kp):
        ...
        while i < len(cc):
            if (scan.full_fsc_only and cc[i] not in _full_fsc_combinations) or (
                any(v == "Acp_KP" for v in [scan.var_x, scan.var_y]) and plot_acp_kp and int(cc[i]) >= 1000
            ):
...
def parse_args():
    ...
    parser.add_argument(
        "--scan-dy-rs",
        action="store_true",
        help="Provide predictions for the value of DY(K- pi+).",
    )
    ...
    if args.no_dcs_cpv is not None:
        assert len(args.no_dcs_cpv) == len(args.combinations), (
            "You should tell whether CPV is allowed in DCS decays for each combination"
        )
