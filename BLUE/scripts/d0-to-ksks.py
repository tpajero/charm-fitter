...
COMBINATIONS: dict[str, Combination] = {
    "2024": Combination(
        ...
        ["CLEO", "LHCb 2015", "LHCb 2021", "CMS", "Belle + Belle II 2024"],
    ),
    "2025": Combination(
        Measurement("World average 2025", -0.17e-2, 0.62e-2, 0.18e-2, is_average=True),  # p-value 0.99%
        ["CLEO", "LHCb 2015", "LHCb 2021", "CMS", "Belle + Belle II 2025", "LHCb 2025", "LHCb average 2025"],
    ),
}
...
def get_measures(comb: str) -> list[Measurement]:
    ...
    try:
        ...
        sys.exit(
            f"ERROR: Combination {comb!r} not available. Available combinations: {list(COMBINATIONS.keys())}. Exiting..."
        )
    return [MEASUREMENTS[name] for name in combination.members] + [combination.average]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-all",
        action="store_true",
        help="Run all available combinations (ignores --date).",
    )
    ...
    if args.run_all:
        for comb in COMBINATIONS:
            for arxiv, pub in [(False, False), (True, False), (False, True)]:
                plot_average(comb, args.outdir, arxiv=arxiv, pub=pub)
    else:
