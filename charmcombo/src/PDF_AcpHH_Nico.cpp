...
PDF_AcpHH_Nico::PDF_AcpHH_Nico(const theory_config th_cfg, const FSC fsc) : PDF_Abs{9}, th_cfg{th_cfg}, fsc{fsc} {
  name = "Charm_AcpHH_Nico";
  initParameters();
  initRelations();
  initObservables();
...
void PDF_AcpHH_Nico::initParameters() {
  ...
  case theory_config::superweak:
    ...
    break;
  default:
    std::cout << "PDF_AcpHH_Nico::initParameters : ERROR : "
...
void PDF_AcpHH_Nico::initRelations() {
  ...
  add_acpkk(theory, "acp_d0_to_kk_nico_th", 1.685 * d0_lifetime);
}

void PDF_AcpHH_Nico::initObservables() {
  ...
  observables->add(*(new RooRealVar("acp_d0_to_kk_nico_obs", "#it{A_{CP}(K^{+}K^{#minus})} Nico", 0, -1, 1)));
}

void PDF_AcpHH_Nico::setObservables(TString c) {
  ...
  setObservable("acp_d0_to_kk_nico_obs", 4.988e-2);  // 0.063872 - 1.685 * 0.391465 * 0.021211
}

void PDF_AcpHH_Nico::setUncertainties(TString c) {
...
void PDF_AcpHH_Nico::setCorrelations(TString c) {
  corSource = "https://cds.cern.ch/record/2799916/";

  std::vector<double> dataStat = {
      // clang-format off
      1., 0.36, 0., 0.,   0.23, 0.,   0., 0.,   0.   ,  // ACP(KK) Run 1 mu
          1.,   0., 0.,   0.,   0.24, 0., 0.,   0.   ,  // ACP(KK) Run 1 prompt
                1., 0.05, 0.,   0.,   0., 0.06, 0.135,  // ACP(KK) Run 2 CDP
                    1.,   0.,   0.,   0., 0.08, 0.177,  // ACP(KK) Run 2 CDS
                          1.,   0.,   0., 0.,   0.   ,  // DeltaACP Run 1 mu
                                1.,   0., 0.,   0.   ,  // DeltaACP Run 1 prompt
                                      1., 0.,   0.   ,  // DeltaACP Run 2 mu
                                          1.,   0.   ,  // DeltaACP Run 2 prompt
                                                1.   ,  // Nico Run 2
      // clang-format on
  };
  ...
  std::vector<double> dataSyst = {
      // clang-format off
      1., 1., 0., 0.,   0.40, 0., 0., 0., 0.,  // ACP(KK) Run 1 mu
          1., 0., 0.,   0.,   0., 0., 0., 0.,  // ACP(KK) Run 1 prompt
              1., 0.28, 0.,   0., 0., 0., 0.,  // ACP(KK) Run 2 CDP
                  1.,   0.,   0., 0., 0., 0.,  // ACP(KK) Run 2 CDS
                        1.,   0., 0., 0., 0.,  // DeltaACP Run 1 mu
                              1., 0., 0., 0.,  // DeltaACP Run 1 prompt
                                  1., 0., 0.,  // DeltaACP Run 2 mu
                                      1., 0.,  // DeltaACP Run 2 prompt
...
void PDF_AcpHH_Nico::buildPdf() {
  pdf = new RooMultiVarGaussian("pdf_" + name, "pdf_" + name, *(RooArgSet*)observables, *(RooArgSet*)theory, covMatrix);
}

void PDF_AcpHH_Nico::add_acpkk(RooArgList* theory, TString name, double avg_time) {
...
void PDF_AcpHH_Nico::add_dacp(RooArgList* theory, TString name, double avg_time_kk, double avg_time_pp) {
  theory->add(*(Utils::makeTheoryVar(
      name, name,
      boost::str(boost::format("   Acp_KK + %.5f * " + CharmUtils::get_dy_expression(th_cfg, fsc, "KK") +
