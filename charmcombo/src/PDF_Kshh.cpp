...
PDF_Kshh::PDF_Kshh(TString measurement_id, const theory_config& th_cfg)
    ...
    switch (th_cfg) {
        ...
        case theory_config::theoretical:
            ...
            theory->add(*(Utils::makeTheoryVar(
                            ...
                            "           - pow(2 * x12 * y12 * sin(phiM - phiG),2), 0.5), 0.5)  ", parameters)));
            theory->add(
                    *(Utils::makeTheoryVar(
                            "phi_th", "phi_th",
                            "-0.5 * TMath::ATan("
        ...
        case theory_config::superweak:
            ...
            theory->add(
                    *(Utils::makeTheoryVar(
                            ...
                            "    / pow(                                                      "
                            "        pow(pow(x12,2) + pow(y12,2),2)                      "
                            "      - pow(2 * x12 * y12 * sin(phiM),2), 0.5), 0.5)", parameters)));
            theory->add(
                    *(Utils::makeTheoryVar(
                            "phi_th", "phi_th",
                            "-0.5 * TMath::ATan("
                            "       pow(x12,2) * sin(2*phiM)"
                            "    / (pow(x12,2) * cos(2*phiM) + pow(y12,2)))", parameters)));
            break;
        default:
