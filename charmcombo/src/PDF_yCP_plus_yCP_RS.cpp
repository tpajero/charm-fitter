...
void PDF_yCP_plus_yCP_RS::initParameters(const theory_config& th_cfg)
{
...
void PDF_yCP_plus_yCP_RS::initRelations(const theory_config& th_cfg)
{
...
void PDF_yCP_plus_yCP_RS::initObservables(const TString& setName)
{
    ...
    observables->add(*(new RooRealVar(
            "yCP_plus_yCP_RS_obs",
            setName + "   #it{y_{CP}}+#it{y_{CP}^{K^{#minus}#pi^{+}}}",
            0, -1e4, 1e4)));
}
