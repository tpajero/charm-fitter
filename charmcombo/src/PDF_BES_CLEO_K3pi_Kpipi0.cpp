...
PDF_BES_CLEO_K3pi_Kpipi0::PDF_BES_CLEO_K3pi_Kpipi0(TString measurement_id)
    : PDF_Abs{6}
{
...
void PDF_BES_CLEO_K3pi_Kpipi0::initObservables(const TString& label) {
    ...
    observables->add(*(new RooRealVar("Delta_K3pi_obs",   label + "   #it{#delta_{K3#pi}}", 0, DegToRad(-180), DegToRad(180))));
    observables->add(*(new RooRealVar("k_Kpipi0_obs",     label + "   #it{#kappa_{K#pi#pi^{0}}}", 1, -2, 2)));
    observables->add(*(new RooRealVar("Delta_Kpipi0_obs", label + "   #it{#delta_{K#pi#pi^{0}}}", 0, DegToRad(-180), DegToRad(180))));
