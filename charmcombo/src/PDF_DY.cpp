...
void PDF_DY::initRelations() {
    ...
    if (nObs == 1) {
        ...
        theory->add(*(Utils::makeTheoryVar("DY_KK_th", "DY_KK_th", CharmUtils::get_dy_expression(th_cfg, fsc, "KK"), parameters)));
        theory->add(*(Utils::makeTheoryVar("DY_PP_th", "DY_PP_th", CharmUtils::get_dy_expression(th_cfg, fsc, "PP"), parameters)));
    }
}
...
void PDF_DY::initObservables(const TString& setName) {
    ...
    if (nObs == 1) {
        ...
        observables->add(*(new RooRealVar("DY_KK_obs", setName + "   #Delta#it{Y}_{#it{K}^{+}#it{K}^{#minus}}", 0, -1e4, 1e4)));
        observables->add(*(new RooRealVar("DY_PP_obs", setName + "   #Delta#it{Y}_{#it{#pi}^{+}#it{#pi}^{#minus}}", 0, -1e4, 1e4)));
    }
}
...
void PDF_DY::setObservables(TString c) {
    ...
    if (c.EqualTo("truth")) setObservablesTruth();
        ...
        cout << "PDF_DY::setObservables() : ERROR : config " << c
            << " not found for " << nObs << " DY observables." << endl;
        exit(1);
    }
}
...
void PDF_DY::setUncertainties(TString c) {
    ...
    if (nObs == 1 && c.EqualTo("WA2019"))           { StatErr[0] =  2.6e-2;  SystErr[0] = 0.;      }
        ...
        StatErr[0] = 2.35e-2; SystErr[0] = 0.57e-2;
        StatErr[1] = 4.30e-2; SystErr[1] = 0.70e-2;
    } else if (nObs == 2 && c.EqualTo("WA2021")) {
        StatErr[0] = 1.28e-2; SystErr[0] = 0.32e-2;
        StatErr[1] = 2.36e-2; SystErr[1] = 0.39e-2;
    } else {
        cout << "PDF_DY::setUncertainties() : ERROR : config " << c
            << " not found for " << nObs << " DY observables." << endl;
        exit(1);
    }
}
...
void PDF_DY::setCorrelations(TString c) {
    ...
    if (nObs == 1) corSource = "No correlations for one observable";
    else if (nObs == 2 && c.EqualTo("WA2020")) corSystMatrix[0][1] = 0.63;  // np.sum(np.square([0.05, 0.42, 0.10, 0.04, 0.23, 0.09])) / 0.57 / 0.70
    else if (nObs == 2 && c.EqualTo("WA2021")) corSystMatrix[0][1] = 0.68;  // np.sum(np.square([0.18, 0.21, 0.06, 0.01, 0.07])) / 0.32 / 0.39
    else {
        cout << "PDF_DY::setCorrelations() : ERROR : config " << c
            << " not found for " << nObs << " DY observables." << endl;
        exit(1);
    }
}
